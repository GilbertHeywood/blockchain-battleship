...
contract('BattleShipGame', async (accounts) => {

  it("should not have any players defined when the contract is created", async () => {
    let battleship = await BattleShipGame.deployed();
    ...
    let state = await battleship.gameState.call();
    // let numLogs = result.logs.length;
    
    battleship = await BattleShipGame.deployed();
  ...
  it("the players should be assigned when a new game starts", async () => {
    let battleship = await BattleShipGame.deployed();
    let result = await battleship.setPlayers(accounts[0],accounts[1],{from: accounts[0]});
    let player1 = await battleship.player1.call();
  ...
  it("the players shouldn't be able to start the game yet", async () => {
    let battleship = await BattleShipGame.deployed();
    try{
      await battleship.startSettingUp();
    }catch(e){

    }
    let state = await battleship.gameState.call();
    assert.equal(state, 1, "The game started without both players putting in funds");
  });
  ...
  it("the players should be able to start the game", async () => {
    ...
    await battleship.startSettingUp();
    let state = await battleship.gameState.call();
    assert.equal(state, 2, "The game didn't started when funds were in the account");
  });
  ...
  it("the players should be able start game onces all pieces put down", async () => {
    ...
    assert.equal(state,3,"Game not able to start even though pieces are down");
